...
void UCombatBehaviour::BehaviourEnter_Implementation(UBaseGoalData* GoalData)
{
	...
	if (M_GoalData == nullptr || M_GoalData->SelfTargets.Num() == 0)
	{
		M_BehaviourState = BehaviourExecutionState::FAILED;
		return;
	}
	M_TargetActor = M_GoalData->SelfTargets[0];
...
void UCombatBehaviour::BehaviourTick_Implementation(float DeltaTime)
{
	...
	else
	{
		if (M_CanShoot)
		{
			if (AShooterBaseCharacter* targetCharacter = Cast<AShooterBaseCharacter>(M_GoalData->SelfTargets[0]))
			{
